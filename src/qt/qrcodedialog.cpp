...
{
    QString ret = QString("legitcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LGT unit input in lnReqAmount, we generate the URI with LGT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LGT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {

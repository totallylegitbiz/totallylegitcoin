...
{
    QString ret = QString("totallylegitcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TLC unit input in lnReqAmount, we generate the URI with TLC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::TLC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {

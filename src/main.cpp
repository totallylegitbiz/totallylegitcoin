...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uLGT
...
void ThreadScriptCheck() {
    RenameThread("legitcoin-scriptch");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LegitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LegitcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("LegitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("legitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LegitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LegitcoinMiner terminated\n");
        throw;
    }
}

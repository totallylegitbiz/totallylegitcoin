...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uTLC
...
void ThreadScriptCheck() {
    RenameThread("totallylegitcoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1388845783, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xa36431c2 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xa3, 0x64, 0x31, 0xc2 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TotallyLegitCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TotallyLegitCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("TotallyLegitCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("totallylegitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TotallyLegitCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TotallyLegitCoinMiner terminated\n");
        throw;
    }
}
